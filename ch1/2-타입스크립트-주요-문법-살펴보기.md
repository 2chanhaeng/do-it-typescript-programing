...
````javascript
function* genWhile5Func(i = 0) {
  while (i < 5) {
    let i = yield i + 1;
  }
}
const genWhile5 = genWhile5Func();
console.log(genWhile5.next().value); // 1
console.log(genWhile5.next().value); //
console.log(genWhile5.next(-1).value); // 0
console.log(genWhile5.next().value); // 1
console.log(genWhile5.next().value); // 2
